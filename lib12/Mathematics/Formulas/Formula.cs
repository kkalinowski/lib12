...
namespace lib12.Mathematics.Formulas
{
    ...
    public class Formula
    {
        ...
        private List<Token> Parse(string text)
        {
            ...
            for (int i = 0; i < text.Length; i++)
            {
                if (char.IsDigit(text[i])
                    || text[i].ToString() == CultureInfo.CurrentCulture.NumberFormat.NumberDecimalSeparator)
                {
                    token.Clear();
                    token.Append(text[i]);
                    while (text.Length >= i + 2 && (char.IsDigit(text[i + 1])
                        || text[i + 1].ToString() == CultureInfo.CurrentCulture.NumberFormat.NumberDecimalSeparator))
                    {
        ...
        public double Evaluate(object argument = null)
        {
            ...
            var argumentProperties = argument.NotNull() ? argument.GetType().GetTypeInfo().DeclaredProperties : null;
