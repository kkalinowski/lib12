...
namespace lib12.Test.Misc
{
    public class RangeTests
    {
        ...
        {
            var range = new Range<int>(3, 12);
            range.LocateInRange(2).ShouldEqual(LocationInRange.LessThanStart);
            range.LocateInRange(3).ShouldEqual(LocationInRange.OnStart);
            range.LocateInRange(5).ShouldEqual(LocationInRange.InRange);
            range.LocateInRange(12).ShouldEqual(LocationInRange.OnEnd);
            range.LocateInRange(15).ShouldEqual(LocationInRange.GreaterThanEnd);
        }
